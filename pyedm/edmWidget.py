...
class reColorInfo(object):
    ...
    # This optimization fails for controlbutton.py because the same palette is controlled by two different
    # colorinfo variables.
...
class edmWidget(edmWidgetSupport):
    ''' edmWidget - base class for all edm-style widgets.
            ...
            DebugFlag - > 0 to print values to stdout
    ...
    #
    #
    def edmEditWidget(self, index):
        return edmEditWidget(self)

    def getEditMode(self):
        try: return self.editMode
        except: pass    # acceptable failure

        try: return self.edmParent.getEditMode()
        except: pass    # acceptable failure
    ...
    def getSelectedWidget(self):
        try: return self.selectedWidget[0]
        except: pass    # acceptable failure

        try: return self.edmParent.getSelectedWidget()
        except: pass    # acceptable failure

        return None

    edmEditTitle = "Widget"
    edmEditPre = [ edmEdit.Int("x", "x"), edmEdit.Int("y", "y", "ypos"), edmEdit.Int("w", "width"), edmEdit.Int("h", "height") ]
    edmEditList = []
    edmEditPost = []
